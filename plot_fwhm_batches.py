...
for i in range(1, 4):
    for j in range(1, 4):
        ...
        # Calculate averages
        avg_fwhm_x_cmos = np.mean(fwhm_x_cmos[region_name]) if fwhm_x_cmos else None
        avg_fwhm_y_cmos = np.mean(fwhm_y_cmos[region_name]) if fwhm_y_cmos else None
        avg_fwhm_x_ccd = np.mean(fwhm_x_ccd[region_name]) if fwhm_x_ccd else None
        avg_fwhm_y_ccd = np.mean(fwhm_y_ccd[region_name]) if fwhm_y_ccd else None

        # Plot FWHM_X and Y for CMOS
        if args.cam in ['CMOS', 'both'] and fwhm_x_cmos:
        ...
        # Plot FWHM_X and Y for CCD
        if args.cam in ['CCD', 'both'] and fwhm_x_ccd:
