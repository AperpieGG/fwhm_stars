...
for i in range(1, 4):
    for j in range(1, 4):
        ...
        ax.plot(bjds1, fwhm_x_cmos[region_name], 'o', label=f'FWHM_X CMOS, Avg= {avg_fwhm_x_cmos:.2f} μm', color='red', alpha=0.5)
        ax.plot(bjds2, fwhm_x_ccd[region_name], 'o', label=f'FWHM_X CCD, Avg= {avg_fwhm_x_ccd:.2f} μm', color='blue', alpha=0.5)

        # Plot FWHM_Y for CMOS and CCD
        ax.plot(bjds1, fwhm_y_cmos[region_name], 's', label=f'FWHM_Y CMOS, Avg= {avg_fwhm_y_cmos:.2f} μm', color='green', alpha=0.5)
